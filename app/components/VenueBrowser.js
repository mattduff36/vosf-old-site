...
export default function VenueBrowser() {
  ...
  const fetchVenues = async () => {
    try {
      ...
      const response = await fetch(`/api/vosf/venues?${params}`);
      if (!response.ok) {
  ...
  const getGoogleMapsUrl = (venue) => {
    ...
    const lon = parseCoordinate(venue.lon);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          {venueData.statistics.total} professional recording venues in London
        </p>
      </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-green-50 p-12 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              Map showing {venueData.statistics.total} venues centered around London
            </p>
            ...
            <div className="mt-4">
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${venueData.statistics.center.lat},${venueData.statistics.center.lon}`}
                target="_blank"
          ...
          <div key={venue.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-2 pt-4 border-t border-gray-100">
              <div className="flex items-center text-sm text-gray-600">
                ...
                <span>
                  Latitude: {formatCoordinate(venue.lat)}
                </span>
              </div>
              ...
              <div className="flex items-center text-sm text-gray-600">
                ...
                <span>
                  Longitude: {formatCoordinate(venue.lon)}
                </span>
              </div>

              {parseCoordinate(venue.lat) !== null && parseCoordinate(venue.lon) !== null && (
                <div className="flex items-center text-sm text-gray-600">
                  <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span>
                    Coordinates: {formatCoordinate(venue.lat)}, {formatCoordinate(venue.lon)}
                  </span>
                </div>
              )}
            </div>
      ...
      {/* No Results */}
      {venueData.venues.length === 0 && !loading && (
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{venueData.statistics.total}</div>
