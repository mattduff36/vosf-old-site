...
export default function StudioNetwork() {
  ...
  const fetchNetworkData = async () => {
    try {
      ...
      const response = await fetch(`/api/vosf/network?${params}`);
      if (!response.ok) {
  ...
  const getConnectionStatusBadge = (accepted) => {
    if (accepted === 1) {
      return (
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 p-6 rounded-lg border border-yellow-200">
          ...
          <div className="text-3xl font-bold text-yellow-700">
            {networkData.statistics.pending}
          </div>
      ...
      {networkData.topStudios.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg">
          ...
          <div className="divide-y divide-gray-200">
            {networkData.topStudios.map((studio, index) => (
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                      <span className="text-blue-600 font-medium text-sm">
                        {connection.studio1_username.charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <div>
                      <div className="font-medium text-gray-900">{connection.studio1_username}</div>
                      {connection.studio1_name && connection.studio1_name !== connection.studio1_username && (
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center">
                      <span className="text-purple-600 font-medium text-sm">
                        {connection.studio2_username.charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <div>
                      <div className="font-medium text-gray-900">{connection.studio2_username}</div>
                      {connection.studio2_name && connection.studio2_name !== connection.studio2_username && (
