...
export default function FAQBrowser() {
  ...
  const fetchFAQs = async () => {
    try {
      ...
      const response = await fetch(`/api/vosf/faq?${params}`);
      if (!response.ok) {
  ...
  const expandAll = () => {
    if (faqData?.faqs) {
      setExpandedItems(new Set(faqData.faqs.map(faq => faq.id)));
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          {faqData.statistics.total} frequently asked questions and user inquiries
        </p>
      </div>
                  ...
                  <div className="pt-4">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-gray-700 whitespace-pre-wrap">
                        {faq.phone || 'No answer provided.'}
                      </div>
                    </div>
                    
                    <div className="mt-4 flex items-center justify-between text-xs text-gray-500">
                      <span>FAQ ID: {faq.id}</span>
                      <span>Question by: {faq.username}</span>
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{faqData.statistics.total}</div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {search ? faqData.faqs.length : faqData.statistics.total}
            </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <p className="text-blue-800 text-sm mb-4">
          This knowledge base contains historical FAQ data from the original VOSF platform. 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <ul className="text-blue-700 space-y-1">
              <li>• Search by username or question content</li>
              <li>• Use keywords to find specific topics</li>
              <li>• Expand items to read full answers</li>
            </ul>
          </div>
          <div>
            <h3 className="font-medium text-blue-900 mb-2">Data Source:</h3>
            <ul className="text-blue-700 space-y-1">
              <li>• Historical VOSF FAQ database</li>
